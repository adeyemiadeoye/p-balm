...
def solve_mpc(system, mode, sim_steps, callback=None, xi=10, alpha=12, delta=1e-6, tol=1e-5, max_iter=2000,
              verbose=False, callback_steps=[], save_gevals=False):
    x = system.x0
    ...
    for k in range(sim_steps):
        ...
        if save_gevals:
            ...
            gevals_list.append(float(last_eval))

    if save_gevals:
        import os
        archive_path = "grad_evals_archive.npz"
        if os.path.exists(archive_path):
            existing = dict(np.load(archive_path, allow_pickle=True))
        else:
            existing = {}
        existing[gevals_key] = np.array(gevals_list)   # numeric 1D array (len == sim_steps)
...
def build_problem(system, x_current, Np, callback=None):
    ...
    for k in range(N+1):
        base = k*state_dim
        # set per-state bounds from the system; if a system didn't set these they should be +/- inf
